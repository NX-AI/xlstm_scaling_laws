...
# batch dims
B, n_chunk = sp.symbols(
    "B n_chunk", integer=True, positive=True
)

# sequence dims
S, T, L = sp.symbols("S T L", integer=True, positive=True)
# generation and prefill sequence length
T_pre, T_gen = sp.symbols(
    "T_pre T_gen", integer=True, positive=True
)

# number of heads
n_headq, n_headkv, = sp.symbols(
    "n_headq n_headkv", integer=True, positive=True
)

# head dims
d_qk = sp.symbols(
    "d_qk", integer=True, positive=True
)
d_hv = sp.symbols(
    "d_hv", integer=True, positive=True
)
# dimension factor
...
n_blocks = sp.symbols(
    "n_blocks", integer=True, positive=True
)
d_ff = sp.symbols(
    "d_ffn", integer=True, positive=True
)
n_vocab = sp.symbols(
    "n_vocab", integer=True, positive=True
)
...
# flop factors model
F_skip, F_norm = sp.symbols(
    "F_skip F_norm", real=True, positive=True
)
F_swish = sp.symbols(
    "F_swish", real=True, positive=True
)
F_softmax = sp.symbols(
    "F_softmax", real=True, positive=True
)
